...
try:
    ...
    st.error("⚠️ Model file not found! Check 'full_pipeline_compressed.sav'")
...
if st.button("🚀 Generate Full Health Analysis"):
    ...
    # --- FEATURE: BMI HEALTH STATUS ---
    st.markdown("---")
    st.subheader("⚖️ BMI Health Classification")
    if bmi < 18.5:
        st.info(f"Your BMI is **{bmi:.1f}**: Underweight (எடை குறைவு)")
    elif 18.5 <= bmi < 25:
        st.success(f"Your BMI is **{bmi:.1f}**: Normal Weight (சரியான எடை)")
    elif 25 <= bmi < 30:
        st.warning(f"Your BMI is **{bmi:.1f}**: Overweight (அதிக எடை)")
    else:
        st.error(f"Your BMI is **{bmi:.1f}**: Obese (பருமன்)")
    ...
    try:
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"Assessment_Report_{patient_name if patient_name else 'Patient'}.pdf",
